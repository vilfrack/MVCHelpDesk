...
$.FileDialog = function FileDialog(userOptions) {
    ...
    dropfieldInner.css({
		//SE MODIFICAN AMBAS LINEAS DE CODIGOS
        "height": 100,//options.dropheight,
        "padding-top": 100 / 2 - 32
    });
    ...
    var loadFile = function(f) {
        ...
        readers.push(reader);
			...
			//SE VALIDA EL TIPO DE ARCHIVOS
			var archivo = 	f.name;
		var Getextension = archivo.replace(/^.*\./, '');
		var objExt = options.ext;
		$.each(objExt, function (index,value) {
			console.log(value+" "+Getextension);
			if(value==Getextension){
				extensionType = false;
				return extensionType; 
			}
		});
			//FIN DE LA VALIDACION DE LOS ARCHIVOS
			
			
			// countReaders CUENTA CUANTOS ARCHIVOS SE PUEDEN AGREGAR
			var countReaders= false;
			if (readers.length > options.length || extensionType== true )	
			{
				countReaders = true;
			}
		else{
		...
		//FIN DE LA VALIDACION DE LA CANTIDAD DE ARCHIVO
		...
		var progress = "";
		// SE VALIDA SI TENDRA EL PROGRESS BAR ROOJO O AZUL
		if(countReaders){
			 var progress = $([
        ...
        $(".bfd-remove", progress).tooltip({
            ...
            row.fadeOut();
            try { 
			//SE AGREGA EL .SPLICE PARA ELIMINAR EL VALOR DEL ARREGLO SELECCIONADO
